...
## Next steps
- Exercise BDOS console-status polling in the test suite so interactive shells can rely on function `0x0B` without tripping busy-wait loops in the host shim.
- Extend the printer and paper-tape capture path to the `--no-cpm-traps` BIOS shims so full system images can spool output through host-managed files without patching their BDOS vectors.
- Annotate CPMI headers with initial attribute hints so BDOS cold boots can seed the read-only vector before any directory traversal occurs.
