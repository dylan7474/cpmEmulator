...
When mounting images that begin with the 16-byte `CPMI` header, the emulator now extracts the encoded sector size, sectors per track, and optional track count automatically. The two highest bits of the track-count field act as feature flags: bit 31 indicates that the header is followed by a translation-table length and one byte per logical sector describing the BIOS skew order (using the familiar 1-based numbering), while bit 30 signals that a 16-bit little-endian default DMA address is stored immediately after the header and before any translation metadata. The payload immediately following the header (and any optional sections) is treated as the first sector, so existing raw media remain compatible while curated images can embed geometry, translation, and DMA metadata for convenience.
...
## Next steps
- Add an automated regression for the BDOS punch/list devices so paper-tape and printer output remain wired to the host capture paths.
- Plumb the system and archive attribute bits through the host-backed file handles so BDOS refuses writes to protected media without relying solely on directory updates.
- Extend the CPMI header format to describe per-drive directory buffer sizing so BIOS table initialisation can right-size scratch space without manual overrides.
